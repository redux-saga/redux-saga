...
import proc, { NOT_ITERATOR_ERROR, SagaCancellationException, CANCEL } from '../src/proc'
...
test('processor effect cancellation handling: call effect', assert => {
  ...
  const endP = proc(main())
  cancelDef.promise.then(v => {
    actual.push(v)
    endP[CANCEL](new SagaCancellationException())
  })
...
test('processor effect cancellation handling: parallel effect', assert => {
  ...
  const expected = ['start',
    ...
    'cancel',
    'subroutine 1 cancelled', 'subroutine 2 cancelled', 'cancelled']
