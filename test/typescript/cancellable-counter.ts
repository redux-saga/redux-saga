import { take, put, call, fork, race, cancelled } from '../../effects'
import {eventChannel, END, SagaIterator} from '../../index';

declare const actions: any;
const { INCREMENT_ASYNC, INCREMENT, CANCEL_INCREMENT_ASYNC, COUNTDOWN_TERMINATED } = actions;
...
export function* incrementAsync({value}): SagaIterator {
...
export function* watchIncrementAsync(): SagaIterator {
  try {
    while(true) {
      ...
      yield race({
        inc: call(incrementAsync, action),
